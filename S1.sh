#!/bin/bash
region=($(awk -F "\"*,\"*" 'FNR>1{u[$13]+=$NF}END{for(i in u)printf "%6.4f %s \n", u[i], i}' Sample-Superstore.csv | sort -n | head -n1))
state=($(awk -F "\"*,\"*" -v a=${region[1]} 'FNR>1{if($13~a)u[$11]+=$NF}END{for(i in u)printf "%6.4f %s \n", u[i], i}' Sample-Superstore.csv | sort -n | head -n2))

#jika dipisah per state
#prod1=($(awk -F "\"*,\"*" -v a="${state[1]}" 'FNR>1{if($11~a)u[$17]+=$NF}END{for(i in u)printf "%5.4f in %s \\n\n", u[i], i}' Sample-Superstore.csv | sort -n | head -n10))
#prod2=($(awk -F "\"*,\"*" -v b="${state[3]}" 'FNR>1{if($11~b)u[$17]+=$NF}END{for(i in u)printf "%5.4f in %s \\n\n", u[i], i}' Sample-Superstore.csv | sort -n | head -n10))

#jika digabung state dan product
prod12=($(awk -F "\"*,\"*" -v a="Texas" -v b="Illinois" -v c="1" 'FNR>1{if($11~a||$11~b)u[$17]+=$NF}END{for(i in u)printf "%5.4f in %s\\n\n",u[i],i}' Sample-Superstore.csv | sort -n | head -n10 | nl -s"."
))

#jika digabung state namun dipisah per product
#prod12=($(awk -F "\"*,\"*" -v a="${state[1]}" -v b="${state[3]}" 'FNR>1{if($11~a||$11~b)printf "%5.4f %s\n",$21,$17}' Sample-Superstore.csv | sort -n | head -n10))
